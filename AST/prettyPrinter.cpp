...
namespace cosmos::AST{
    ...
    // ------------- Expr printing fns -------------
    namespace{
        auto parenthesize(const std::string& name, const ExprPtrVariant& expr) -> std::string {
            return "(" + name + " " + PrettyPrinter::to_string(expr) + ")";
        }

        auto parenthesize(const std::string& name, const ExprPtrVariant& expr1, const ExprPtrVariant& expr2) -> std::string {
            return "(" + name + " " + PrettyPrinter::to_string(expr1) + " " + PrettyPrinter::to_string(expr2) + ")";
        }
        ...
        auto printLiteralExpr(const LiteralExprPtr& expr) -> std::string {
            return expr->literalVal.has_value() ? Types::get_literal_string(expr->literalVal.value()) : "nil";
        }
        ...
        auto printConditionalExpr(const ConditionalExprPtr& expr) -> std::string {
            return parenthesize(": " + parenthesize("?", expr->condition), expr->thenBranch, expr->elseBranch);
        }

        auto printPostfixExpr(const PostfixExprPtr& expr) -> std::string {
            return parenthesize("POSTFIX " + expr->op.get_lexeme(), expr->left);
        }

        auto printVariableExpr(const VariableExprPtr& expr) -> std::string {
            return "(" + expr->varName.get_lexeme() + ")";
        }       

        auto printAssignmentExpr(const AssignmentExprPtr& expr) -> std::string {
            return parenthesize("= " + expr->varName.get_lexeme(), expr->right) + ";";
        }
        ...
        auto printCallExpr(const CallExprPtr& expr) -> std::string {
            ...
            for (size_t i = 0; i < expr->arguments.size(); ++i) {
                ...
                result += "(" + PrettyPrinter::to_string(expr->arguments[i]) + ")";
            }
        ...
        auto printFuncExpr(const FuncExprPtr& expr) -> std::string {
            ...
            for (size_t i = 0; i < expr->parameters.size(); ++i) {
                if (0 != i) funcStr += ", ";
                funcStr += expr->parameters[i].to_string();
            }
            funcStr += ") {\n";
            for (auto& bodyStmt : expr->body) {
                for (const auto& str : PrettyPrinter::to_string(bodyStmt)) {
                    funcStr += str + "\n";
                }
            }
            funcStr += "\n}";
        ...
        auto printGetExpr(const GetExprPtr& expr) -> std::string {
            std::string str = PrettyPrinter::to_string(expr->expr);
            str += " .( get " + expr->name.get_lexeme() + " )";
            return str;
        }

        auto printSetExpr(const SetExprPtr& expr) -> std::string {
            std::string str = PrettyPrinter::to_string(expr->expr);
            std::string val = PrettyPrinter::to_string(expr->value);
            str += " .( set " + expr->name.get_lexeme() + " ) = " + val;
            return str;
        }
        ...
        auto printSuperExpr(const SuperExprPtr& expr) -> std::string {
            return "( super." + expr->method.to_string() + " )";
        }
    } // namespace


    auto PrettyPrinter::to_string(const ExprPtrVariant& expression) -> std::string {
        switch (expression.index()) {
          ...
          default:
            static_assert(std::variant_size_v<ExprPtrVariant> == 15,
                          ...
                          "PrettyPrinter::to_string(const ExptrVariant&)!");
            return "";
        }
    }
    ...
    namespace {
        ...
        auto printBlockStmt(const BlockStmtPtr& blkStmts) -> std::vector<std::string> {
          ...
          for (auto& stmt : blkStmts->statements) {
            auto stmtStr = PrettyPrinter::to_string(stmt);
            if (!stmtStr.empty())
              std::move(stmtStr.begin(), stmtStr.end(), std::back_inserter(blkStmtStrsVec));
          }
          blkStmtStrsVec.emplace_back("}");
        ...
        auto printVarStmt(const VarStmtPtr& stmt) -> std::string {
          std::string str = "var " + stmt->varName.get_lexeme();
          if (stmt->initializer.has_value()) {
            str = "( = ( " + str + " ) " + PrettyPrinter::to_string(stmt->initializer.value()) + " )";
          }
          return str + ";";
        }
        ...
        auto printIfStmt(const IfStmtPtr& stmt) -> std::vector<std::string> {
          ...
          ifStmtStrVec.emplace_back("( if (" + PrettyPrinter::to_string(stmt->condition) + ")");
          auto thenBranchVec = PrettyPrinter::to_string(stmt->thenBranch);
          std::move(thenBranchVec.begin(), thenBranchVec.end(), std::back_inserter(ifStmtStrVec));
          if (stmt->elseBranch.has_value()) {
            ifStmtStrVec.emplace_back(" else ");
            auto elseBranchVec = PrettyPrinter::to_string(stmt->elseBranch.value());
            std::move(elseBranchVec.begin(), elseBranchVec.end(), std::back_inserter(ifStmtStrVec));
          }
        ...
        auto printWhileStmt(const WhileStmtPtr& stmt) {
          ...
          whileStmtStrVec.emplace_back( "( while (" + PrettyPrinter::to_string(stmt->condition) + ")");
          auto loopBodyVec = PrettyPrinter::to_string(stmt->loopBody);
          std::move(loopBodyVec.begin(), loopBodyVec.end(), std::back_inserter(whileStmtStrVec));
        ...
        auto printForStmt(const ForStmtPtr& stmt) {
          ...
          if (stmt->initializer.has_value()) {
            auto initVec = PrettyPrinter::to_string(stmt->initializer.value());
            std::move(initVec.begin(), initVec.end(), std::back_inserter(forStmtStrVec));
          }
          forStmtStrVec.emplace_back(
              "; "
              + (stmt->condition.has_value()
                     ? PrettyPrinter::to_string(stmt->condition.value())
                     : std::string())
              + ";"
              + (stmt->increment.has_value()
                     ? PrettyPrinter::to_string(stmt->increment.value())
                     : std::string())
              + ")");
        ...
        auto printFuncStmt(const FuncStmtPtr& stmt) -> std::vector<std::string> {
          ...
          funcStrVec.emplace_back("( ( " + stmt->funcName.get_lexeme() + ") ");
        ...
        auto printRetStmt(const RetStmtPtr& stmt) -> std::string {
          std::string value = stmt->value.has_value() ? PrettyPrinter::to_string(stmt->value.value()) : " ";
          return "( return" + value + ");";
        }

        auto printClassStmt(const ClassStmtPtr& stmt) -> std::vector<std::string> {
          std::vector<std::string> strVec;
          strVec.emplace_back("(CLASS " + stmt->className.get_lexeme() + " )");
          if (stmt->superClass.has_value())
            strVec.emplace_back(" < " + PrettyPrinter::to_string(stmt->superClass.value()));
          strVec.emplace_back("{");
          for (const StmtPtrVariant& method : stmt->methods) {
            std::vector<std::string> methodStr = PrettyPrinter::to_string(method);
    ...
    auto PrettyPrinter::to_string(const StmtPtrVariant& statement) -> std::vector<std::string> {
      switch (statement.index()) {
        ...
        default:
          static_assert(
              ...
              "Looks like you forgot to update the cases in "
              "PrettyPrinter::to_string(const StmtPtrVariant& statement)!");
          return {};
      }
    }

    auto PrettyPrinter::to_string(const std::vector<AST::StmtPtrVariant>& statements)
