...
FnStmt::FnStmt(Token identifier, std::vector<Token> params, std::vector<unique_stmt_ptr> body) : identifier{std::move(identifier)}, params{std::move(params)}, body{std::move(body)} {
    assert(this->identifier.type == TokenType::IDENTIFIER);
}
...
IfBranch::IfBranch(unique_expr_ptr condition, unique_stmt_ptr statement) : condition{std::move(condition)}, statement{std::move(statement)} {
...
BreakStmt::BreakStmt(Token keyword) : keyword{std::move(keyword)} {
    assert(this->keyword.type == TokenType::EJECT);
}
...
ContinueStmt::ContinueStmt(Token keyword) : keyword{std::move(keyword)} {
    assert(this->keyword.type == TokenType::WARP);
}
...
VarStmt::VarStmt(Token identifier, unique_expr_ptr initializer) : identifier{std::move(identifier)}, initializer{std::move(initializer)} {
    assert(this->identifier.type == TokenType::IDENTIFIER);
}
...
WhileStmt::WhileStmt(unique_expr_ptr condition, unique_stmt_ptr body) : condition{std::move(condition)}, body{std::move(body)} {
    assert(this->condition != nullptr);
...
ForStmt::ForStmt(unique_stmt_ptr initializer, unique_expr_ptr condition, unique_expr_ptr increment, unique_stmt_ptr body)
