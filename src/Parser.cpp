...
unique_stmt_ptr Parser::statement() {
    if (match({TokenType::NAVIGATE}))
        return forStatement();
    if (match({TokenType::PROBE}))
        return ifStatement();
    if (match({TokenType::FLARE}))
        return printStatement();
    if (match({TokenType::TRANSMIT}))
        return returnStatement();
    if (match({TokenType::ORBIT}))
        return whileStatement();
    if (match({TokenType::LEFT_BRACE}))
        return std::make_unique<BlockStmt>(block());
    if (match({TokenType::EJECT, TokenType::WARP}))
...
unique_stmt_ptr Parser::ifStatement() {
    ...
    return std::make_unique<IfStmt>(std::move(main_branch), std::move(elif_branches), std::move(else_branch));
}
...
unique_expr_ptr Parser::assignment()
{
    ...
    if (match({TokenType::EQUAL}))
    {
        ...
        {
            return std::make_unique<SubscriptExpr>(std::move(subscript_ptr->identifier), std::move(subscript_ptr->index), std::move(value));
        }
...
bool Parser::match(const std::initializer_list<TokenType> token_args) {
    bool is_match = std::any_of(token_args.begin(), token_args.end(), [this](TokenType x) { return check(x); });
    if (is_match) {
        advance();
    }
    return is_match;
